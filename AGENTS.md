...
每个 Notebook 的开头 blockquote 是全文的门面，由两个不同部分组成：
**开场叙述 → 四点提纲**。两部分作用不同，不能互相替代，也不能把四点提纲压缩进叙述段落。

- **开场叙述**：先用 1-2 段普通引用承接上下文、给出具体问题或例子。这里不用粗体标签，不要写 `**前情回顾**：`、`**本节目标**：`；段落之间用一个空 `>` 行分隔
- **四点提纲**：叙述之后必须保留四个分点，概括本章的四组核心内容。每点使用一个简短的粗体名称和一句具体说明，例如 `**训练样本**：shifted label 定义每个位置的正确答案`。可以写成 `1.` 到 `4.` 的编号，也可以写成四个由空 `>` 分隔的引用段落
- **不要合并**：即使四点会在正文中继续展开，也不能删除、合并或改写成一段连续叙述；它们是学习路线，不是开场叙述的重复
